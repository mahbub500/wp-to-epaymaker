...
class Helper {
	...
	public static function get_posts( $args = [], $show_heading = false, $show_cached = false ) {
		...
		// use cache
		if( true === $show_cached && ( $cached_posts = wp_cache_get( "wpm_{$_args['post_type']}", 'wpm' ) ) ) {
			$posts = $cached_posts;
		}
		...
		// don't use cache
			...
			wp_cache_add( "wpm_{$_args['post_type']}", $posts, 'wpm', 3600 );
		}

		$posts = $show_heading ? [ '' => sprintf( __( '- Choose a %s -', 'wpm' ), $_args['post_type'] ) ] + $posts : $posts;

		return apply_filters( 'wpm_get_posts', $posts, $_args );
	}
	 ...
	 *
	 * It'll look into /wp-to-epaymaker directory of your active theme
	 * first. if not found, default template will be used.
	 * can be overwriten with wp-to-epaymaker_template_overwrite_dir hook
	 *
	...
	public static function get_template( $slug, $base = 'views', $args = null ) {
		...
		$overwrite_template_dir = apply_filters( 'wpm_template_overwrite_dir', get_stylesheet_directory() . '/wp-to-epaymaker/', $slug, $base, $args );
		
		// default template directory
		$plugin_template_dir = dirname( WP_TO_PAYMAKER ) . "/{$base}/";
		...
		elseif ( file_exists( $plugin_template_path ) ) {
			...
			return __( 'Template not found!', 'wp-to-epaymaker' );
		}
	}
}